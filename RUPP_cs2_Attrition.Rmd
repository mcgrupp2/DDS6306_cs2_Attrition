...
```{r}
attr_comp_data <- read.csv("data/CaseStudy2CompSetNoAttrition.csv")

cm_fs_df <- dummy_cols(attr_comp_data)

cm_f_df <- cbind(cm_fs_df[,2],
              cm_fs_df[,4],
              cm_fs_df[,6:7],   
              cm_fs_df[,11],
              cm_fs_df[,13:15], 
...
names(cm_f_df)[[1]] <- "Age"
names(cm_f_df)[[2]] <- "DailyRate"
names(cm_f_df)[[5]] <- "EnvironmentSatisfaction"
names(cm_f_df)[[6]] <-  "HourlyRate"
names(cm_f_df)[[9]] <- "JobSatisfaction"

cm_f_df <- cm_f_df %>% mutate(train_labels = NA)

levels(cm_f_df$train_labels) <- c(0,1)

cm_all_scaled_df <- cm_f_df %>% mutate_at(vars(cola), list(~scale(.) %>% as.vector))

cm_trn_lbls <- cm_all_scaled_df$train_labels

cm_pd_mx <- xgb.DMatrix(data = cm_all_scaled_df, label = cm_trn_lbls)

cm_pd_mx <- data.matrix(cm_all_scaled_df)

cm_pred <- predict(xgb_model, cm_pd_mx)

#writing final submission file
submission_select <- data.frame(empID = cm_fs_df[,1], attr = cm_pred)
write.csv(submission_select, file = 'attrition_final.csv', row.names = F)
